...
// Процедура заполняет табличную часть документа возврата книгами, которые на данный момент находятся у абонента
&НаКлиенте
Процедура ЗаполнитьПоЗадолженности(Команда)
	
	Если Не ЗначениеЗаполнено(Объект.Абонент) Тогда
		Сообщить(НСтр("ru = 'Не заполнен реквизит абонент'"));
		Возврат;
	КонецЕсли;
	
	Если Вопрос(НСтр("ru = 'Будет очищена табличная часть. Продолжить?'"), РежимДиалогаВопрос.ДаНет,0) = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
