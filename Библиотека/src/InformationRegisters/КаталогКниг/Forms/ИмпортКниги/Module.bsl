...
Процедура ЗавершитьИмпортНаСервере()
	...
	Если ЗначениеЗаполнено(СсылкаНаКартинку) Тогда
		ИмяВременногоФайла = ОбщегоНазначенияСервер.ЗагрузитьФайлПоСсылке(СсылкаНаКартинку, ".jpg");
		ФайлНаДиске = Новый Файл(ИмяВременногоФайла);
		Если ФайлНаДиске.Существует() Тогда
			КороткоеИмяФайла = РаботаСФайламиКлиентСервер.КороткоеИмяФайлаИзПолногоПути(ИмяВременногоФайла);
	...
	ДокументПоступления.Комментарии = НСтр("ru = 'Импорт книги из каталога книг'");
	Попытка
		ДокументПоступления.Записать(РежимЗаписиДокумента.Проведение);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		СообщениеПользователю = Новый СообщениеПользователю;
		СообщениеПользователю.Текст = НСтр("ru = 'Не удалось создать документ поступления книг'");
		СообщениеПользователю.Сообщить();
		ОбщегоНазначенияСервер.ЗафиксироватьОшибку(СообщениеПользователю.Текст, ОписаниеОшибки, Метаданные.Документы.ПоступлениеКниг, Наименование);
	КонецПопытки;
