...
// УникальныйИдентификатор	- УникальныйИдентификатор	- Уникальный идентификатор формы, из которой вызывается функция. Нужен чтобы потом стереть файл.
//
...
Процедура ПеренестиФайл(Файл, МестоНазначения) Экспорт
	...
	Если МестоНазначения = Перечисления.МестаХраненияФайлов.ЛокальныйДиск Тогда 
		...
		Исключение
			...
			ОбщегоНазначенияСервер.ЗафиксироватьОшибку(НСтр("ru = 'Произошла ошибка при переносе файла на локальный диск.'"), ОписаниеОшибки, Метаданные.Справочники.Файлы, Файл);
			Возврат;
		КонецПопытки;
	...
	Иначе
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ОбщегоНазначенияСервер.ЗафиксироватьОшибку(НСтр("ru = 'Произошла ошибка при переносе файла в информационную базу.'"), ОписаниеОшибки, Метаданные.Справочники.Файлы, Файл);
			Возврат;
		КонецПопытки;
...
// Создает файл на диске и возвращает его имя
//
// Параметры:
...
Функция СоздатьФайлНаДиске(КороткоеИмяФайла, ДанныеФайла, ИдентификаторФайла) Экспорт
	...
	Исключение 
		ОписаниеОшибки = ОписаниеОшибки();
		ОбщегоНазначенияСервер.ЗафиксироватьОшибку(НСтр("ru = 'Произошла ошибка при сохранении файла на диске.'"), ОписаниеОшибки, Метаданные.Справочники.Файлы);
