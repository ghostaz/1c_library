...
Функция ЗагрузитьФайлПоСсылке(СсылкаНаФайл, РасширениеФайла = Неопределено) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияСервер.ЗафиксироватьОшибку(СообщениеПользователю.Текст, ОписаниеОшибки + Символы.ПС + "Ссылка: " + СсылкаНаФайл, Метаданные.РегистрыСведений.КаталогКниг, Неопределено);
	...
	Если Не Файл.Существует() Тогда
		...
		СообщениеПользователю.Текст = "Не удалось загрузить файл по ссылке";
		СообщениеПользователю.Сообщить();
		ОбщегоНазначенияСервер.ЗафиксироватьОшибку(СообщениеПользователю.Текст, "Ссылка: " + СсылкаНаФайл, Метаданные.РегистрыСведений.КаталогКниг, Неопределено);
		Возврат Неопределено;
	КонецЕсли;
