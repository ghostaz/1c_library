...
Процедура ОбновитьСтатусРегистрации()
	...
	Если ЗначениеЗаполнено(УникальныйИдентификаторБазы) Тогда
		СтатусРегистрации = НСтр("ru = 'Информационная база зарегистрирована'");
		ЭтаФорма.Элементы.СтатусРегистрации.ЦветТекста = Новый Цвет(0,150,0);
	Иначе
		СтатусРегистрации = НСтр("ru = 'Информационная база не зарегистрирована'");
...
Процедура ЗарегистрироватьБазу(Команда)
	Если ЗарегистрироватьБазуСервер() = 0 Тогда
		СтатусРегистрации = НСтр("ru = 'Произошла ошибка при выполнении операции. Повторите попытку позже.'");
		ЭтаФорма.Элементы.СтатусРегистрации.ЦветТекста = Новый Цвет(150,0,0);
	Иначе
		ОбновитьСтатусРегистрации();
	КонецЕсли;		
...
Процедура ПроверитьОбновлениеСервер()
	...
	Если РезультатКод = 0 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Произошла ошибка при выполнении операции. Повторите попытку позже.'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,0,0);
	ИначеЕсли РезультатКод = 1 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Имеется обновление'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(0,150,0);
	ИначеЕсли РезультатКод = 2 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Новых обновлений нет'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,150,0);
	ИначеЕсли РезультатКод = 3 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Уникальный идентификатор базы не заполнен. Для пользования сервисом необходима регистрация.'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,0,0);
	ИначеЕсли РезультатКод = 4 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'База не найдена в списке зарегистрированных'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,0,0);
	ИначеЕсли РезультатКод = 5 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Неизвестна версия информационной базы'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,0,0);
	ИначеЕсли РезультатКод = 6 Тогда
		РезультатПроверкиОбновления 	= НСтр("ru = 'Информационная база не зарегистрирована. Для пользования сервисом необходима регистрация.'");
		ЭтаФорма.Элементы.РезультатПроверкиОбновления.ЦветТекста = Новый Цвет(150,0,0);
	КонецЕсли;
